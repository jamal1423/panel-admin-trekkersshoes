    ...
    <div class="container-fluid">
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb bg-white mb-0 px-0 py-2">
          <li class="breadcrumb-item active" aria-current="page">Produk Populer</li>
        </ol>
      </nav>
    </div>
  </div>
                    ...
                    <div class="table-responsive" id="printableTable">
                      <div id="productTable_wrapper" class="dataTables_wrapper no-footer">
                        <div id="productTable_filter" class="dataTables_filter">
                          <form action="{{ url('/produk-populer-cari') }}" method="post">
                            @method('post')
                            @csrf
                            <label>
                              Cari:
                        ...
                        <tbody class="kt-table-tbody text-dark">
                          @if($singleProduk == "yes")
                            @forelse($dataProdukPopuler as $produk)
                            ...
                            @endforelse
                          @else
                            @forelse($dataProdukPopuler as $key=>$produk)
                            <tr class="kt-table-row kt-table-row-level-0 odd" role="row">
                              <td class="sorting_1">{{ $loop->iteration }}</td>
                              <td>
                                <div class="d-flex align-items-center">
