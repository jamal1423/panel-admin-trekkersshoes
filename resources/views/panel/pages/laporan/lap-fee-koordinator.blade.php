    ...
    <div class="container-fluid">
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb bg-white mb-0 px-0 py-2">
          <li class="breadcrumb-item active" aria-current="page">Lap. Fee Koordinator</li>
        </ol>
      </nav>
    </div>
  </div>
              ...
              <div class="card card-custom gutter-b bg-white border-0">
                <div class="card-header align-items-center border-0">
                  <div class="card-title">
                      <h3 class="card-label font-weight-bold text-body text-center">Laporan Fee Koordinator</h3>
                      <label>
                        Periode : 
                        @if($tglAwal == "" && $tglAkhir == "")
                          -
                  ...
                  <div class="row">
                    <div class="col-12 gutter-pt table-padding" style="padding:35px;">
                      <table class="table text-body">
                        <thead>
                          <tr class="row"> 
                            <th class="border-0 col-lg-2 col-3 header-heading" scope="col">Bulan Order</th>
                            <th class="border-0 col-lg-4 col-4 header-heading" scope="col">Koordinator</th>
                            <th class="border-0 col-lg-3 col-2 text-center header-heading" scope="col">Qty</th>
                            <th class="border-0 col-lg-3 col-3 text-right header-heading" scope="col">Total Jual</th>
                          </tr>
                        </thead>
                        <tbody>
                          @php
                          ...
                          <tr class="row">
                            <td class="col-lg-2 col-3 font-size-bold">{{ date('F Y', strtotime($data->tgl_order)) }}</td>
                            <td class="col-lg-4 col-4 font-size-bold">
                              {{ $data->jns_member }}
                              @foreach($admMitra as $mitra)
                                @if($mitra->adm_mitra == $data->jns_member)
                                  <span class="badge badge-info">{{ $mitra->nama }}</span>
                                @endif
                              @endforeach
                            </td>
                            <td class="col-lg-3 col-2 font-size-bold text-center">{{ $data->totQty }}</td>
                            <td class="col-lg-3 col-3 font-size-bold text-right">{{ "Rp. ".number_format($data->totBeli) }}</td>
                          </tr>
                          <tr class="row">
                            <td colspan="3" class="col-lg-10 col-3 font-size-bold text-right">Grand Total</td>
                            <td class="col-lg-2 col-2 font-size-bold text-right">{{ "Rp. ".number_format($data->totBeli) }}</td>
                          </tr>
                          <tr class="row">
                            <td colspan="3" class="col-lg-10 col-3 font-size-bold text-right">Fee Total (3%)</td>
                            <td class="col-lg-2 col-2 font-size-bold text-right" style="background-color:#5ef88c;">{{ "Rp. ".number_format($data->totBeli * (3/100)) }}</td>
                          </tr>
                          @endforeach
                        </tbody>
                      </table>
