                    ...
                    <div class=" table-responsive" id="printableTable">
                      <div id="productTable_wrapper" class="dataTables_wrapper no-footer">
                        ...
                        <tbody class="kt-table-tbody text-dark">
                          @foreach($transaksiStockistVerifikasi as $transaksi)
                          <tr class="kt-table-row kt-table-row-level-0 odd" role="row">
                            <td class="sorting_1">{{ $loop->iteration }}</td>
                            <td>
                              <a href="/manifest-verifikasi/detail/{{ base64_encode($transaksi->id_order) }}/{{ base64_encode($transaksi->status_approval) }}/{{ base64_encode($transaksi->status) }}">
                            ...
                            <td>
                              @if($transaksi->status_approval == "N")
                                ...
                                @elseif($transaksi->status == 'Verifikasi Nota')
                                  <span class="badge badge-pill badge-warning">{{ $transaksi->status }}</span> <span class="badge badge-pill badge-danger">Un-Approved</span>
                                @else
                                  <span class="badge badge-pill badge-success">{{ $transaksi->status }}</span>
                                @endif
                              @else
                              <span class="badge badge-pill badge-warning">{{ $transaksi->status }}</span> <span class="badge badge-pill badge-success">Approved</span>
                              @endif
                            </td>
                          </tr>
                          @endforeach
