    ...
    <div class="container-fluid">
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb bg-white mb-0 px-0 py-2">
          <li class="breadcrumb-item active" aria-current="page">Transaksis</li>
        </ol>
      </nav>
    </div>
  </div>
                    ...
                    <div class=" table-responsive" id="printableTable">
                      <div id="productTable_wrapper" class="dataTables_wrapper no-footer">
                        ...
                        <tbody class="kt-table-tbody text-dark">
                          @foreach($dataTransaksiBatal as $transaksi)
                          <tr class="kt-table-row kt-table-row-level-0 odd" role="row">
                            <td class="sorting_1">{{ $loop->iteration }}</td>
                            <td>
                              <a href="/transaksi-selesai/detail/{{ base64_encode($transaksi->id_order) }}">
                            ...
                            <td>
                              ...
                              @else
                              <span class="badge badge-pill badge-primary">{{ $transaksi->status }}</span>
                              @endif
                            </td>
                          </tr>
                          @endforeach
