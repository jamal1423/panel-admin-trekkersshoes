                    ...
                    <div class=" table-responsive" id="printableTable">
                      <div id="productTable_wrapper" class="dataTables_wrapper no-footer">
                        ...
                        <tbody class="kt-table-tbody text-dark">
                          @foreach($transaksiStockistProses as $transaksi)
                          <tr class="kt-table-row kt-table-row-level-0 odd" role="row">
                            <td class="sorting_1">{{ $loop->iteration }}</td>
                            <td>
                              @if($transaksi->status_approval == "")
                                @php $approve = 'X' @endphp
                              @endif
                              <a href="/manifest-proses/detail/{{ base64_encode($transaksi->id_order) }}/{{ base64_encode($approve) }}/{{ base64_encode($transaksi->status) }}">
